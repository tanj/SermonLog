# -*- coding: utf-8 -*-
<%!
import urllib.parse

from faapp.model.meta import get_pk_map
%>
<thead>
  <tr>
    %for field in collection.render_fields.values():
      <th>${field.label()|h}</th>
    %endfor
  </tr>
</thead>

<tbody>
%for i, row in enumerate(collection.rows):
  <%
  collection._set_active(row)
  %>
  <tr class="${i % 2 and 'odd' or 'even'}">
  %for field in collection.render_fields.values():
    <td>${field.render_readonly()|n}</td>
  %endfor
  <td><a href="${ request.resource_url(request.context, urllib.parse.urlencode(get_pk_map(row))) }">${ _("Edit") }</a>
  <td><a href="${ request.resource_url(request.context, urllib.parse.urlencode(get_pk_map(row)), "delete") }">${ _("Delete") }</a>
  </tr>
%endfor
</tbody>
<br>

